# myweb — AWS EC2 컨테이너 배포 파이프라인

Nginx 리버스 프록시 뒤에 Docker 컨테이너로 정적 웹을 서비스하고,
GitHub Actions로 자동 배포되는 구조를 직접 구축한 실습 프로젝트입니다.
...
| 영역 | 사용 기술 |
|---|---|
| 인프라 | AWS EC2 (Amazon Linux), Elastic IP, Security Group |
...
### 인프라
- EC2 인스턴스 프로비저닝 및 SSH 키 기반 접속 구성
- Security Group으로 인바운드 제어 (22 / 80 / 443)
- Elastic IP로 고정 주소 확보 후 DNS A 레코드 연결
- Certbot으로 인증서 발급, HTTP → HTTPS 리다이렉트, 자동 갱신 타이머 검증

### 컨테이너
- Dockerfile로 애플리케이션 이미지화 (`nginx:alpine` 기반, 69MB)
...
- **보안** — 현재 GitHub Actions 접속을 위해 22번 포트를 전체 개방. AWS Systems Manager 또는 self-hosted runner로 전환 필요
- **무중단 배포** — 컨테이너 교체 중 짧은 다운타임 발생. 블루-그린 방식 적용
- **IaC** — 현재 인프라는 콘솔 수동 구성. Terraform으로 코드화
